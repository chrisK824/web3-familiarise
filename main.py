...
SEPOLIA_ETH_CHAIN_ID = 11155111
SEPOLIA_ETH__RPC_NODE = "https://rpc.sepolia.org"
w3 = Web3(Web3.HTTPProvider(SEPOLIA_ETH__RPC_NODE))
eth = Eth(w3)

def create_account():
    account = Account.create()
    print(f"Account address: {account.address} with private key: {account.key}")
...
def get_balance():
    ...
    if not account:
        account = input("No default account found, please provide with an account address:\n")
...
def send_transaction(recipient, ether):
    ...
    if not sender:
        sender = str(input("No default account found for sender, please provide with an account sender address:\n"))
...
def get_acc_txns_count():
    account = eth.default_account
    if not account:
        account = str(input("No default account found, please provide with an account address:\n"))
...
def get_block(block_id, full_transactions):
    block = eth.get_block(block_identifier=block_id, full_transactions=full_transactions)
...
print(f"Client is {'connected' if w3.is_connected() else 'not connected'} to RPC node {SEPOLIA_ETH__RPC_NODE}")
...
while True:
    if selection == 1:
        ...
        block_id = input("Insert a block by integer or one of keywords latest/earliest/pending/safe/finalized\n")
        try:
